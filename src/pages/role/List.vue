  ...
  <div class="card">
    ...
    <div class="form-container">
      ...
      >
        <template #default-filters>
          <tr>
            <td class="col-6" data-label="Name">
              <p class="grouped">
                <label class="hide-mobile" for="">Name</label>
                <input type="text" name="name" placeholder="Filter by name...">
              </p>
            </td>
            <td class="col-6" data-label="Description">
              <p class="grouped">
                <label class="hide-mobile" for="">Description</label>
      ...
      <!-- No Roles State -->
      <div v-else-if="!hasItems && !loading" class="no-roles">
            ...
            <td class="column-action" data-label="Actions">
              <div class="action-table">
                ...
                <TableAction
                  ...
                  title="Edit"
                  @click="handleEdit(item, ROLE_ROUTES.EDIT_ROLE)"
                />
                <TableAction
                  variant="danger"
                  icon="bi bi-trash"
    ...
    <footer class="content-footer">
      <div class="footer-actions">
        ...
        <FormButton variant="danger" @click="deleteSelectedRoles" text="Delete" :disabled="!hasSelectedItems" />
        <FormButton variant="success" @click="handleCreate(ROLE_ROUTES.CREATE_ROLE)" text="+ Create" />
      </div>
    </footer>

    <!-- Role Detail Modal -->
    <Show v-if="showModal" :role="selectedItem" @edit="handleEdit" @close="showModal = false" />
  </div>
</template>
...
// Use the global list composable with our custom role pagination
const {
  filterComponent,
  items,
  loading,
  ...
  handleRouteUpdate,
  initialize
} = useGlobalList(ROLE_API_ROUTES, useRolePagination, useAlert)
...
// Define search options based on Role fields
const searchOptions = [
...
// Delete selected roles function (specific to role list)
async function deleteSelectedRoles() {
  await deleteSelected(http, ROLE_API_ROUTES.remove)
}

// Refresh data when route is updated (e.g. when navigating back from role form)
onBeforeRouteUpdate(handleRouteUpdate)

// Initialize
onMounted(() => {
  console.log('Role list mounted. Current HTTP baseURL:', http.baseURL)
